 ...
 *  It's a sqlite database that can be opened with Sqlite Database Browser.
 * 	It can be downloaded here: http://sourceforge.net/projects/sqlitebrowser/?source=dlp
 *
...
$(document).ready(function() {
	fbless_backend_init();
	console.log("[FBless] Backend initialized.");
});
...
function fbless_backend_init() {
	...
	persistence.schemaSync(null, function(tx) {
		console.log("[FBless] Backend schema sync'ed.");
	});
	...
	Attention.save = function(attn, should_not_flush) {
		...
		console.log("[FBless] attention saved!");
		if (!should_not_flush) {
			persistence.transaction(function(tx) {
				persistence.flush(tx, function() {
					console.log('[FBless] persistence flushed');
				});
			});
		}
	};
